...
Legitimate large evidence (unique long reasoning, full transcripts, large JSON deliverables) that survives compaction can still exceed the cap. When the server advertises result-evidence support, such a run records through the out-of-band path below instead of dying as the execution error above; against a server without the advertisement, the bounded rejection remains the outcome — never silent truncation.

### Out-of-Band Result Evidence

Document-heavy runs upload their oversized result fields as attempt-scoped staging parts and finalize by reference (issue #251):

1. **Advertisement** — the same lease responses that carry `result_max_bytes` also carry `result_evidence_supported`, `result_evidence_max_item_bytes`, and `result_evidence_max_total_bytes` (from `APO_RESULT_EVIDENCE_MAX_ITEM_BYTES` / `APO_RESULT_EVIDENCE_MAX_TOTAL_BYTES`). Absent fields mean an older server: the client keeps the inline-only contract.
2. **Staging uploads** — `POST /v1/executor-protocol/{v1,v2}/attempts/{id}/result-evidence` opens an intent for a slot (`transcript`, `checks`, or `deliverable` + name) with the exact `size_bytes` and `sha256` of the identity-encoded JSON bytes; `PUT /v1/executor-protocol/result-evidence/{evidence_id}` streams those bytes through the configured `ArtifactStore`, which verifies size and digest before the row becomes `ready`. The PUT path is version-neutral (opaque id addressing) and authenticated by the same Attempt JWT, matched against the part's own attempt — a foreign attempt gets an opaque 404.
3. **Finalization by reference** — `/result` may carry `evidence_refs` instead of the corresponding inline fields. Finalization resolves each ref against the same attempt, re-verifies the logical digest of the bytes it reads, parses the JSON, and lands it in the same permanent store an inline result uses: the transcript into `transcript_json`, deliverables through `persist_json_deliverable` (inline ≤64 KiB, gzip+object beyond), checks through `persist_check_report`. Every existing reader — run detail, export, comparison, deliverable download — works unchanged.
4. **Idempotency** — the stored completion digest hashes the *wire* body (refs, not resolved contents), so a replay after staging cleanup matches. The completion precheck runs before refs are resolved, so an idempotent replay never needs the (now deleted) parts. Interrupted PUTs are retry-safe: identical intent metadata returns the same id, and a re-PUT of a ready part is a no-op success.
5. **Staging is transient** — successful finalization, failure finalization, run deletion, and a daily TTL (`APO_RESULT_EVIDENCE_STALE_TTL_SECONDS`) all remove staging rows and objects; the orphan-object reaper counts live evidence keys as referenced. No second permanent store exists, so retention/backup cover nothing new.
6. **Client policy** — `apo task run` and `apo connect` upload parts only when the measured body is over the advertised cap, transcript first, then deliverables largest-first, then checks, stopping as soon as the body fits; uploads run inside the heartbeat window that already spans artifact uploads and the terminal POST (issue #176). A part larger than the per-item cap, or a body still over the limit after externalization, falls back to the bounded `result_too_large:` rejection.
